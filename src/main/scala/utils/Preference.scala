...
object Preference extends SWTHelper
{
  ...
  def readEmotes()
  {
    ...
    for (emote <- Source.fromFile(emoteFile).getLines()) {
      ...
      for (image <- loadFromFile(imageFile)) {
  ...
  def readAvatars()
  {
    ...
    Source.fromFile(avatarFile).getLines().foreach { avatar =>
      ...
      TwitchUser.addAvatar(nickname, imageFile)
    }
  }
  ...
  def read(setting: BlockSetting)
  {
    ...
    setting.transparentLabel.setText(setting.alphaTitle + transparent + "%")

    setting.messageSizeSpinner.setSelection(preference.getInt("BlockMessageSize", 10))
  ...
  def save(setting: BlockSetting)
  {
    ...
    preference.putInt("BlockTransparent", setting.transparentScale.getSelection)
  ...
  def save(setting: BalloonSetting)
  {
    ...
    preference.putInt("BalloonTransparent", setting.transparentScale.getSelection)
  ...
  {
    val portText = Option(preference.get("IRCPort", "6667").trim).filterNot(_.isEmpty).getOrElse("6667") 
